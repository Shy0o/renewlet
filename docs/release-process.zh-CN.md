...
- `Cloudflare Worker`：fork 用户或维护者测试环境的自管 Worker 部署。官方仓库只从 `dev` 部署，稳定生产 Cloudflare 部署仍只走 `Release Publish`；缺少 Cloudflare secrets 时只跳过远端部署，配置 secrets 后才执行 D1 migration 和 Worker deploy。
...
Cloudflare 测试部署使用 `Cloudflare Worker` workflow。在 `zhiyingzzhou/renewlet` 官方仓库里，deploy job 只允许 `dev` 执行，避免 `main` 合并后用 `packageVersion-dev+shortSha` 构建覆盖生产 Worker。fork 和自管仓库仍可从 `dev`、`main` 或 GitHub Actions 手动运行部署。这个 workflow 面向测试环境或用户自管 Worker 环境，和正式版发布的生产审批门分开。
