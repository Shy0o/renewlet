...
如果页面提示 `Failed to get repository contents` / `无法获取存储库内容`，通常是 Cloudflare 读取 GitHub 公共仓库时被临时限流。不要连续重试；稍后再试，或改走 [Cloudflare Workers 手动部署](docs/cloudflare-workers-deploy.zh-CN.md)。
